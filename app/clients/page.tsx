...
export default function Clients() {
  ...
  return (
    <div>
      <Header title="Clients" subtitle="Portfolio, health & last-quarter sentiment — click a client for detail" />
                      ...
                      <td colSpan={8} className="px-6 py-5">
                        <div className="grid lg:grid-cols-2 gap-5">
                          <div>
                            ...
                            <div className="text-xs text-mav-muted">LTV {fmtUsd(c.ltv_usd || 0)} · {bk.length} booking(s) in range · {esc.length} escalation(s) · {fb.length} feedback</div>
                          </div>
                          <div className="space-y-4">
                            {bk.length > 0 && <Section title={`Bookings (${bk.length})`}>{bk.slice(0, 8).map(b => <Row key={b.id} a={b.booking_month?.slice(0, 7) || b.booking_date} b={b.service_name} c={b.booking_amount ? fmtUsd(b.booking_amount) : ''} />)}</Section>}
                            ...
                            {fb.length > 0 && <Section title={`Feedback (${fb.length})`}>{fb.slice(0, 5).map(x => <Row key={x.id} a={x.nature} b={(x.comments || '').slice(0, 80)} c="" />)}</Section>}
                            ...
                            {sig.length > 0 && <Section title={`Email signals (${sig.length})`}>{sig.slice(0, 5).map(x => <Row key={x.id} a={x.sentiment} b={(x.summary || x.source_subject || '').slice(0, 80)} c="" />)}</Section>}
