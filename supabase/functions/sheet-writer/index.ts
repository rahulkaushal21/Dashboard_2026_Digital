...
async function buildRevenue(sb: any): Promise<string[][]> {
  ...
  const { data: opps, error: oe } = await sb.from("opportunities")
    .select("id, quote_key, company_name, source_subject, client_name, contact_email, client_type, service_dept, service_type, delivery_type, delivery_status, project_type, technology, geo, pm_owner, sales_person, business_type, currency, quote_price, local_value, est_value, start_date, delivery_date, confirmed_at, source_date, origin")
  ...
  for (const o of opps || []) {
    ...
    // The sheet's own Project Id is PRJ + the Quote ID's digits, generated by the Apps
    // Script at quote time. Reuse those digits where the deal carries a real quote id;
    // otherwise issue PRJ-D<id>, which cannot collide with a timestamp-shaped one and
    // says plainly that the dashboard issued it. That shape is also what marks a row as
    // dashboard-written, now that Project Status carries the real delivery state.
    const qut = /QUT(\d+)/i.exec(s(o.quote_key));
    put(col.projectId, qut ? `PRJ${qut[1]}` : `PRJ-D${o.id}`);
    put(col.quote, s(o.quote_key));
    ...
    // Nothing the dashboard books is outsourced; the column is 99.8% zero in the source.
    put(col.outsourceUsd, "0.00");
